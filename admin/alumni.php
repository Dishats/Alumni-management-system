					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$alumni = $conn->query("SELECT a.*,c.course,Concat(a.lastname,' ',a.firstname,' ',a.middlename) as name from alumnus_bio a inner join courses c on c.id = a.course_id order by Concat(a.lastname,'s ',a.firstname,' ',a.middlename) asc");
...
<script>
    ...
    $('.send_mail').click(function(){
        ...
        if(confirm("Are you sure you want to send a verification email to " + email + "?")) {
            $.ajax({
                url: 'send_email.php',  // Create this file
                method: 'POST',
                data: { email: email, id: alumniId },
